...
function hasLinearConfig(): boolean {
  return Boolean(LINEAR_ENV.apiKey && LINEAR_ENV.teamKey);
}
...
function fromLinearIssue(issue: LinearIssue): AgentTask {
  ...
  return {
    ...
    dodChecklist: checklistItems.map((item) => item.label),
    evidence,
    approvalLog,
...
async function getTeamId(): Promise<string> {
  ...
  const data = await linearGraphQL<{ team: { id: string } | null }>(query, {
    teamKey: LINEAR_ENV.teamKey,
  });

  if (!data.team?.id) {
    throw new Error("Linear team not found for LINEAR_DEFAULT_TEAM.");
  }
...
async function listLinearIssues(): Promise<LinearIssue[]> {
  ...
  }>(query, { teamKey: LINEAR_ENV.teamKey });
...
async function getLinearIssueById(id: string): Promise<LinearIssue> {
  ...
  const data = await linearGraphQL<{ issue: LinearIssue | null }>(query, { id });
  if (!data.issue) {
